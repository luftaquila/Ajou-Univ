...
#define LIFE 10													//체력 초기 값

///////////////////////////////// 몰라도 상관없는 코드
void printWord(int x, int y, char* word, char isColor) {					//입력받은 x, y 좌표에 입력받은 word 출력
	gotoxy(x, y);
...
int main(void) {
	...
	*/
	...
	printWord(0, REMAIN_SPECIAL_Y, tempstring, FALSE);

	//////////////////////////////////////////////////////////////////
	//몰라도 상관없는 코드
	printWord(0, DESCRIPTION_Y, "끝내려면 '종료'를 입력하세요.", FALSE);
	...
	*/
	...
	while (run) {
		...
		for (itr = 0; itr < remainLife * 8; itr++)
			printWord(itr++, MAX_Y, "■", FALSE);							//닿으면 끝나는 부분 표시
	...
	printWord(0, MAX_Y + 1, "종료합니다.", FALSE);
	gotoxy(0, MAX_Y + 2);
	return 0;
}
...
void removeRaindropFromConsol(RAINDROP_LIST * raindropList, int idx) {
	...
	for (itr = 0; itr < raindropList->raindrops[idx].length; itr++)
		tempstring[itr] = ' ';
	tempstring[itr] = '\0';
	printWord(raindropList->raindrops[idx].x, raindropList->raindrops[idx].y, tempstring, FALSE);
	/////////////////////////////////
}
